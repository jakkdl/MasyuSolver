...
class Solver():
    ...
    # cell). Then we will follow that line, until we come to another cell with only one line
    # (the ending cell).
    def __processSubPaths_orig(self, puzzleBoard):
        changesMade = False
        numRows, numCols = puzzleBoard.getDimensions()
        for rowNum in range(0, numRows):
            for colNum in range(0, numCols):
                ...
                if (numLines == 1):
                    ...
                    if (l):
                        # Proceed to the cell to the left (cell(rowNum, colNum-1))
                        # print("starting:", rowNum, "x", colNum)
                        result = self.__moveToNextCell(puzzleBoard, rowNum, colNum - 1, rowNum, colNum,
                                                       numCirclesVisited)
                        ...
                        # Proceed to the cell above (cell(rowNum-1, colNum))
                        # print("starting:", rowNum, "x", colNum)
                        result = self.__moveToNextCell(puzzleBoard, rowNum - 1, colNum, rowNum, colNum,
                                                       numCirclesVisited)
                    ...
                    if (self.__cellsAbut(startingRow, startingCol, endingRow, endingCol)):
                        ...
                        if (numCirclesInPuzzle == numCirclesVisited):
                            ...
                            # Draw the line connecting the two cells
                            changesMade = changesMade | self.__drawLineBetweenCells(puzzleBoard, startingRow,
                                                                                    startingCol, endingRow,
                                                                                    endingCol)

                        else:
                            # Path must be blocked!
                            changesMade = changesMade | self.__blockPathBetweenCells(puzzleBoard, startingRow,
                                                                                     startingCol, endingRow,
                                                                                     endingCol)
